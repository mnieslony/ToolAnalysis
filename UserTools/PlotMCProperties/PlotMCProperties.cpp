#include "PlotMCProperties.h"

PlotMCProperties::PlotMCProperties():Tool(){}


bool PlotMCProperties::Initialise(std::string configfile, DataModel &data){
  ...
  particleE_secondaries = new std::vector<double>;
  ...
  secparticle_posX = new std::vector<double>;
  secparticle_posY = new std::vector<double>;
  secparticle_posZ = new std::vector<double>;
  secparticle_stopposX = new std::vector<double>;
  secparticle_stopposY = new std::vector<double>;
  secparticle_stopposZ = new std::vector<double>;
  ...
  t->Branch("E_true_sec",&particleE_secondaries);
  ...
  t->Branch("SecParticlePosX",&secparticle_posX);
  t->Branch("SecParticlePosY",&secparticle_posY);
  t->Branch("SecParticlePosZ",&secparticle_posZ);
  t->Branch("SecParticleStopPosX",&secparticle_stopposX);
  t->Branch("SecParticleStopPosY",&secparticle_stopposY);
  t->Branch("SecParticleStopPosZ",&secparticle_stopposZ);
...
bool PlotMCProperties::Execute(){
  ...
  std::cout <<"PlotMCProperties got EventFV = "<<event_fv<<" from RecoEvent store"<<std::endl;
  m_data->Stores["RecoEvent"]->Get("NoPiK",no_pik);
  m_data->Stores["RecoEvent"]->Get("EventPMTVol",event_pmtvol);
  std::cout <<"PlotMCProperties got EventPMTVol = "<<event_pmtvol<<" from RecoEvent store"<<std::endl;
  ...
  m_data->Stores.at("ANNIEEvent")->Get("NumVetoHits",numVeto);
  ...
  m_data->Stores.at("ANNIEEvent")->Get("ParticleId_to_MrdCharge", ParticleId_to_MrdCharge);
m_data->Stores.at("ANNIEEvent")->Get("TrackId_to_MCParticleIndex",trackid_to_mcparticleindex);

  std::map<int, std::map<unsigned long, double>>::iterator it;

  std::cout <<"MRD Particle map: "<<std::endl;
  for (it = ParticleId_to_MrdTubeIds->begin(); it != ParticleId_to_MrdTubeIds->end(); it++){
    int particleID = it->first;
    std::map<unsigned long, double> temp_map = it->second;
    std::map<unsigned long, double>::iterator it2;
    std::cout <<"particle ID: "<< particleID << std::endl;
    for (it2 = temp_map.begin(); it2 != temp_map.end(); it2++){
      unsigned long chankey = it2->first;
      double charge = it2->second;
      std::cout <<"chankey: "<<chankey<<", charge: "<<charge<<std::endl;
      if (trackid_to_mcparticleindex->count(particleID)!=0){
        std::cout <<"MCParticleIndex: "<<trackid_to_mcparticleindex->at(particleID)<<std::endl;	
      }
    }   
  }
  ...
  secparticle_posX->clear();
  secparticle_posY->clear();
  secparticle_posZ->clear();
  secparticle_stopposX->clear();
  secparticle_stopposY->clear();
  secparticle_stopposZ->clear();
  particle_dirX->clear();
  particle_dirY->clear();
  particle_dirZ->clear();
  pmtQ->clear();
  ...
  for (unsigned int i_particle = 0; i_particle < mcparticles->size(); i_particle++){
	...
	particleE_secondaries->push_back(particle_energy);
   	secparticle_posX->push_back(particleposX);
	secparticle_posY->push_back(particleposY);
	secparticle_posZ->push_back(particleposZ);
	secparticle_stopposX->push_back(particlestopposX);
	secparticle_stopposY->push_back(particlestopposY);
	secparticle_stopposZ->push_back(particlestopposZ);
    } 
  ...
  if(MCLAPPDHits){
    ...
    for (std::pair<unsigned long, std::vector<MCLAPPDHit>>&& apair : *MCLAPPDHits){
      ...
      if(det==nullptr){
        if (verbosity > 0) std::cout <<"PlotMCProperties Tool: LAPPD Detector not found! "<<std::endl;;
        continue;
      }
    ...
    Log("PlotMCProperties tool: No MCLAPPDHits!", v_warning, verbosity);
    num_lappds_hit = 0;
  }
  ...
  hQtotal_LAPPD->Fill(lappdQtotal);
    ...
    if(!TDCData){
        std::cout<<"PlotMCProperties tool: No TDC data to process!"<<std::endl;
    } else {
        if(TDCData->size()==0){
          Log("PlotMCProperties tool: No TDC hits.",v_message,verbosity);
          hMRDPaddles->Fill(0);
          hMRDLayers->Fill(0);
          mrdPaddles = 0;
          mrdLayers = 0;
...
bool PlotMCProperties::Finalise(){
