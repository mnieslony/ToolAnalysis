...
class PhaseIIADCHitFinder : public Tool {
  ...
  protected:
    ...
    // Keep track of the channelkeys that had waveforms so far
    ...
    std::map<uint64_t, std::map<unsigned long,std::vector<Hit>>*> *InProgressHits;     //Key: {MTCTime}, value: map of  H
    std::map<uint64_t, std::map<unsigned long,std::vector<Hit>>*> *InProgressHitsAux;  //Key: {MTCTime}, value: map of  H
    std::map<uint64_t, std::map<unsigned long,std::vector<std::vector<ADCPulse>>>> *InProgressRecoADCHits; //Key: {MTCTim
    std::map<uint64_t, std::map<unsigned long,std::vector<std::vector<ADCPulse>>>> *InProgressRecoADCHitsAux; //Key: {MTC
    std::map<uint64_t, std::vector<unsigned long>> *InProgressChkey;   //Key: {MTCTime}, value: vector of in progress cha
